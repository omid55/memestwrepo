...
public:
  ...
  // Omid55
  TStr GetSeriesPlotStrPointsAndSmooth(const int& PlotN);
  // Omid55
  int IsSameXCol(const int& CurId, const int& PrevId) const;
  void CreatePlotFile(const TStr& Comment = TStr(), bool withPointsAndSmooth=false);
...
public:
  ...
  void SaveEps(const TStr& FNm, bool pointsAndSmooth, const int& FontSz=30, const TStr& Comment=TStr());  // Omid55
