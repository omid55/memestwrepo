...
// Omid55
TStr TGnuPlot::GetSeriesPlotStrPointsAndSmooth(const int& SeriesId)
{
  TStrV colors;
  colors.Add("red");
  colors.Add("green");
  ...
  colors.Add("grey");
  colors.Add("lime");
  IAssert(SeriesId < colors.Len());   // allways check that this condition has been met
  TStr color = colors[SeriesId];
  ...
  PlotStr += " notitle with " + GetSeriesTyStr(Series.SeriesTy) + " lt rgb \"" + color + "\"";
  ...
  PlotStr += " title \"" + Series.Label + "\" smooth csplines lt rgb \"" + color + "\"";
...
// Omid55
void TGnuPlot::SaveEps(const TStr& FNm, bool pointsAndSmooth, const int& FontSz, const TStr& Comment) {
  AddCmd(TStr::Fmt("set terminal postscript enhanced eps %d color", FontSz));
  AddCmd(TStr::Fmt("set output '%s'", FNm.CStr()));
  Pause(false);
  if(pointsAndSmooth)
  {
	  CreatePlotFile(Comment.Empty()? Title : Comment,true);
  }
  else
  {
	  CreatePlotFile(Comment.Empty()? Title : Comment);
...
void TGnuPlot::CreatePlotFile(const TStr& Comment, bool withPointsAndSmooth) {
  ...
  if (! SeriesV.Empty()) {
    ...
    for (int i = 0; i < SeriesV.Len(); i++)
    {
    	if(withPointsAndSmooth)
    	{
    		fprintf(F, "%s", GetSeriesPlotStrPointsAndSmooth(i).CStr());
    	}
    	else
    	{
