...
void TNIBs::SaveInferredDynamicGephi(const TStr& OutFNm, const TFltV& Steps, const bool& lwcc, const int& NodesOn, const int& SpainOn, const TIntV& NIdV) {
	TFOut FOut(OutFNm);
	bool verbose = true;

	THashSet<TStr> NewsMedia, NewsMediaSpain;

	TFIn FInNewsMedia("news-media.txt");
	TStr Line;
	while (!FInNewsMedia.Eof()) {
		FInNewsMedia.GetNextLn(Line); if (Line=="") { break; }
		NewsMedia.AddKey(Line);
	}

	if (SpainOn==1) {
		TFIn FInNewsMediaSpain("news-media-spanish.txt");
		TStr Line;
		while (!FInNewsMediaSpain.Eof()) {
			FInNewsMediaSpain.GetNextLn(Line); if (Line=="") { break; }
			NewsMediaSpain.AddKey(Line);
		}
	}

	if (verbose) { printf("Writing nodes...\n"); }
	...
	FOut.PutStr("<graph mode=\"dynamic\" defaultedgetype=\"directed\" timeformat=\"date\">\n\n");
	FOut.PutStr("\t<nodes>\n\n");
	for (THash<TInt, TNodeInfo>::TIter NI = NodeNmH.BegI(); NI < NodeNmH.EndI(); NI++) {
		if (NIdV.Len() > 0 && !NIdV.IsIn(NI.GetKey())) { continue; }
		if (SpainOn==1 && NewsMedia.IsKey(NI.GetDat().Name) && !NewsMediaSpain.IsKey(NI.GetDat().Name)) { continue; }

		TStr NodeStr(TStr::Fmt("\t\t<node id=\"%d\" label=\"%s\">\n\n", NI.GetKey().Val, NI.GetDat().Name.CStr()));

		if (NewsMedia.IsKey(NI.GetDat().Name) || (SpainOn==1 && NewsMediaSpain.IsKey(NI.GetDat().Name))) { NodeStr += "\t\t<viz:color r=\"255\" g=\"0\" b=\"0\" a=\"1.0\" />\n"; }
		else { NodeStr += "\t\t<viz:color r=\"0\" g=\"0\" b=\"255\" a=\"1.0\" />\n"; }

		NodeStr += TStr::Fmt("\t\t<attvalues>\n\t\t\t<attvalue for=\"volume\" value=\"%d\"/>\n\t\t</attvalues>\n", NI.GetDat().Vol.Val);

		TStrFltFltHNEDNet::TNodeI NNI = InferredNetwork.GetNI(NI.GetKey());
		double CurrentTime=-1; int k = 0;
		for (int t=0; t<Steps.Len(); t++) {
			if (lwcc) {
				PStrFltNEDNet NetworkAtT;
				GetInferredNetworkAtT(Steps[t], NetworkAtT);
				if (TSnap::GetMxWcc(NetworkAtT)->IsNode(NI.GetKey())) { CurrentTime = Steps[t]; }
			} else {
				// check if the in-degree or out-degree is > 0 for time Steps[t]
				for (int i=0; i<NNI.GetOutDeg(); i++) {
					if (NNI.GetOutEDat(i).IsKey(Steps[t]) && NNI.GetOutEDat(i).GetDat(Steps[t]) > MinAlpha) {
						CurrentTime = Steps[t]; k++; break;
					}
				}

				for (int i=0; i<NNI.GetInDeg(); i++) {
					if (NNI.GetInEDat(i).IsKey(Steps[t]) && NNI.GetInEDat(i).GetDat(Steps[t]) > MinAlpha) {
						CurrentTime = Steps[t]; k++; break;
					}
				}
			}
		...
		if (CurrentTime > 0) {
			// update spell, we assume steps is in hours (TODO: implement any time unit)
			if (NodesOn) {
				NodeStr += TStr::Fmt("\t\t<spells>\n\t\t\t<spell start=\"%s\" ", TSecTm((uint)(Steps[0]*3600.0)).GetStr(tmuDay).CStr());
			} else {
				NodeStr += TStr::Fmt("\t\t<spells>\n\t\t\t<spell start=\"%s\" ", TSecTm((uint)(CurrentTime*3600.0)).GetStr(tmuDay).CStr());

			}
			NodeStr += TStr::Fmt("end=\"%s\" />\n\t\t</spells>\n", TSecTm((uint)(Steps[Steps.Len()-1].Val*3600.0)).GetStr(tmuDay).CStr());
			FOut.PutStr(NodeStr);
			FOut.PutStr("\t\t</node>\n");
			printf("Node %d (%s) written!\n", NI.GetKey().Val, NI.GetDat().Name.CStr());
		}
	}

	FOut.PutStr("\n\t</nodes>\n\n");

	if (verbose) { printf("Nodes written!\n"); }
	if (verbose) { printf("Writing edges...\n"); }
	...
	for (TStrFltFltHNEDNet::TEdgeI EI = InferredNetwork.BegEI(); EI < InferredNetwork.EndEI(); EI++) {
		...
		if (!NodeNmH.IsKey(EI.GetSrcNId()) || !NodeNmH.IsKey(EI.GetDstNId())) { continue; }
		if (SpainOn==1 && (
				(NewsMedia.IsKey(NodeNmH.GetDat(EI.GetSrcNId()).Name) && !NewsMediaSpain.IsKey(NodeNmH.GetDat(EI.GetSrcNId()).Name))
				|| (NewsMedia.IsKey(NodeNmH.GetDat(EI.GetDstNId()).Name) && !NewsMediaSpain.IsKey(NodeNmH.GetDat(EI.GetDstNId()).Name)) )
			) { continue; }

		TStr EdgeStr(TStr::Fmt("\t\t<edge source=\"%d\" target=\"%d\">\n\t\t<spells>\n", EI.GetSrcNId(), EI.GetDstNId()));

		bool LastTime=false; bool CurrentTime=false; int k = 0;
		for (int t=0; t<Steps.Len(); t++) {
			CurrentTime = false;
			...
			if (lwcc) {
				PStrFltNEDNet NetworkAtT;
				GetInferredNetworkAtT(Steps[t], NetworkAtT);
				if (TSnap::GetMxWcc(NetworkAtT)->IsEdge(EI.GetSrcNId(), EI.GetDstNId()) &&
				    EI().IsKey(Steps[t]) && EI().GetDat(Steps[t])>MinAlpha) { CurrentTime = true; k++; }
			} else {
				if (EI().IsKey(Steps[t]) && EI().GetDat(Steps[t])>MinAlpha) { CurrentTime = true; k++; }
			}

			// update spell, we assume steps is in hours (TODO: implement any time unit)
			if (!LastTime && CurrentTime) { EdgeStr += TStr::Fmt("\t\t\t<spell start=\"%s\" ", TSecTm((uint)(Steps[t].Val*3600.0)).GetStr(tmuDay).CStr()); }
			if (LastTime && !CurrentTime) { EdgeStr += TStr::Fmt("end=\"%s\" />\n", TSecTm((uint)(Steps[t].Val*3600.0)).GetStr(tmuDay).CStr()); }

			LastTime = CurrentTime;
		}

		if (CurrentTime) { EdgeStr += TStr::Fmt("end=\"%s\" />\n", TSecTm((uint)(Steps[Steps.Len()-1].Val*3600.0)).GetStr(tmuDay).CStr()); }
	...
	if (verbose) { printf("Edges written!\n"); }
